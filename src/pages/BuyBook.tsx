...
const BuyBook: React.FC<BookDetailsProps> = ({ match }) => {
  ...
  const handlePlayAudio = () => {
    if (book && book.id) {
      // Redirect to ListenToBook page with book ID
      history.push(`/listen-to-book/${book.id}`);
    }
  };
  const playAudioBook = () => {
    if (book && book.audioUrl) {
      ...
      audio.play().catch((error) => {
        console.error('Error playing the audio book:', error);
        alert('Error playing the audio book. Please try again later.');
      });
    }
  };
    ...
    <IonPage>
      ...
      <IonContent className="ion-padding">
        {book ? (
          ...
          <button className="content-btn" onClick={handleViewBook}>View Book</button>
          <button className="content-btn" onClick={handlePlayAudio}>Play Audio Book</button>
        </div>
        ) : (
